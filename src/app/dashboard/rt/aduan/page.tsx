...
export default function RTAduanPage() {
  ...
  const fetchAduan = async () => {
    setLoading(true);
    try {
      // Filter by RT and RW of the RT staff
      const data = await apiFetch(`/aduan?rt=${user?.rt}&rw=${user?.rw}`);
      setAduan(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleProcessAduan = async () => {
    ...
    try {
      await apiFetch(`/aduan/${selectedAduan.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ status: statusUpdate, balasan: balasan, foto_selesai: fotoSelesai })
      });
  ...
  return (
    <div className="flex min-h-screen bg-[#0f172a] text-slate-200">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <h1 className="text-3xl font-bold mb-8">Aduan Warga RT {user.rt}</h1>
