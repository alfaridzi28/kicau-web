...
export default function RWDashboard() {
  ...
  const maxWarga = Math.max(...rtValues, 1);
    ...
    <div className="flex min-h-screen bg-[#0f172a] text-slate-200">
      ...
      <main className="flex-1 p-8 overflow-auto relative min-w-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12 animate-in fade-in slide-in-from-bottom duration-700">
           <StatCard title="Total Penduduk" value={stats?.total_warga || 0} icon="👥" color="emerald" subtitle="Seluruh Wilayah RW" />
           ...
           <StatCard title="Aset Kolektif" value="32" icon="🏪" color="cyan" subtitle="Inventaris Wilayah" />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
             {/* Distribution Chart (Simple HTML/CSS) */}
             <div className="bg-slate-800/40 backdrop-blur-xl rounded-[40px] p-10 border border-white/5 shadow-2xl">
                <div className="flex justify-between items-center mb-12">
                   ...
                   <button className="bg-white/5 hover:bg-white/10 text-[10px] font-black text-slate-400 uppercase tracking-widest px-4 py-2 rounded-xl transition">Detail Wilayah</button>
                </div>
                
                <div className="flex items-end justify-between h-64 gap-6 px-4">
                   {Object.keys(stats?.warga_per_rt || {}).map((label, i) => (
                     <div key={label} className="flex-1 flex flex-col items-center group">
                        <div className="relative w-full flex flex-col items-center">
                           <div className="absolute -top-8 text-[10px] font-black text-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity">{(stats?.warga_per_rt as any)?.[label] || 0} Jiwa</div>
                           <div 
                             ...
                             style={{ height: `${(((stats?.warga_per_rt as any)?.[label] || 0) / maxWarga) * 100}%` }}
                           ></div>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   <div className="space-y-6">
                      <div className="p-8 bg-gradient-to-br from-slate-900 to-slate-800 rounded-[32px] border border-white/5 shadow-inner">
                         <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Total Saldo Kas</p>
                         <p className="text-4xl font-black text-white leading-none">Rp 12.450.000</p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                         <div className="p-6 bg-emerald-500/10 rounded-3xl border border-emerald-500/10">
                            <p className="text-[9px] font-black text-emerald-500 uppercase mb-1">Pemasukan</p>
                            <p className="text-sm font-black text-white">+Rp 2.1M</p>
                         </div>
                         <div className="p-6 bg-red-500/10 rounded-3xl border border-red-500/10">
                            <p className="text-[9px] font-black text-red-500 uppercase mb-1">Pengeluaran</p>
                            <p className="text-sm font-black text-white">-Rp 450K</p>
                         </div>
                      </div>
                   </div>
                   <div className="flex flex-col justify-center gap-4">
                      <p className="text-sm text-slate-400 leading-relaxed font-medium">Monitoring dana iuran dari seluruh RT di wilayah RW {user.rw}. Klik detail untuk melihat laporan per unit RT.</p>
                      <a href="/dashboard/rw/iuran" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-2xl text-center uppercase tracking-widest text-[10px] shadow-lg shadow-blue-900/40 transition-all hover:scale-[1.02]">Laporan Keuangan RT →</a>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="space-y-10">
             ...
             <div className="bg-gradient-to-br from-emerald-600 to-teal-800 rounded-[40px] p-10 shadow-2xl relative overflow-hidden group">
                ...
                <h3 className="text-2xl font-black text-white mb-8 tracking-tighter">RW Control Hub</h3>
                <div className="grid gap-4 relative z-10">
                   ...
                   <a href="/dashboard/rw/aset" className="bg-white/10 hover:bg-white/20 p-6 rounded-2xl border border-white/10 flex items-center gap-5 transition group/btn">
                      <span className="text-2xl group-hover/btn:rotate-12 transition-transform">🏪</span>
                      <span className="font-bold text-white uppercase tracking-widest text-[11px]">Inventaris RW</span>
                   </a>
                   <a href="/dashboard/rw/bansos" className="bg-white/10 hover:bg-white/20 p-6 rounded-2xl border border-white/10 flex items-center gap-5 transition group/btn">
                      <span className="text-2xl group-hover/btn:rotate-12 transition-transform">🤝</span>
                      <span className="font-bold text-white uppercase tracking-widest text-[11px]">Rekap Sosial</span>
                   </a>
                </div>
             </div>
             ...
             {/* Region Status */}
             <div className="bg-slate-800/40 backdrop-blur-xl rounded-[40px] p-10 border border-white/5 shadow-2xl">
                <h3 className="text-xl font-black text-white mb-8">Status Wilayah</h3>
                <div className="space-y-8">
                   ...
                   <div className="pt-6 border-t border-white/5">
                      <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl">
                         ...
                         <p className="text-[10px] font-black text-white uppercase tracking-widest">Sistem Real-Time Online</p>
                      </div>
                   </div>
                </div>
             </div>
